...
type CotacaoReprovadaProps = {
  onNavigateBack?: () => void;
};

export function CotacaoReprovada({ onNavigateBack }: CotacaoReprovadaProps) {
  const { t } = useLanguage();
  const [items] = useState<Item[]>(initialItems);
  const [selectedPatient, setSelectedPatient] = useState("Ana Paula Monteiro");
  ...
  return (
    <section
      aria-labelledby="cotacao-reprovada-heading"
      className="mx-auto flex w-full max-w-5xl flex-col gap-6"
    >
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h1
            id="cotacao-reprovada-heading"
            className="text-3xl font-semibold"
          >
            <span className="text-primary">{t("page.cotacao")}</span>{" "}
