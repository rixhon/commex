...
type NegociacaoEmAnaliseProps = {
  onNavigateBack?: () => void;
  onNavigateToCotacaoGanha?: () => void;
  onNavigateToCotacaoReprovada?: () => void;
  onNavigateToNegociacao?: () => void;
};

export function NegociacaoEmAnalise({
  onNavigateBack,
  onNavigateToCotacaoGanha,
  onNavigateToCotacaoReprovada,
  onNavigateToNegociacao
}: NegociacaoEmAnaliseProps) {
  const { t } = useLanguage();
  ...
  return (
    <section
      aria-labelledby="negociacao-em-analise-heading"
      className="mx-auto flex w-full max-w-5xl flex-col gap-6"
    >
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h1
            id="negociacao-em-analise-heading"
            className="text-3xl font-semibold text-primary"
          >
            {t("page.negociacao-em-analise")}
          </h1>
        </div>
      ...
      <div className="rounded-2xl border border-borderNeutral bg-white shadow-sm">
                ...
                return (
                  <tr key={item.id} className="hover:bg-[#f5f8fb]">
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="mx-auto inline-flex h-8 w-12 items-center justify-center rounded border border-borderNeutral bg-white text-sm text-[#4f4f4f]">
                        {item.quantity}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <div className="mx-auto inline-flex h-8 min-w-[100px] items-center justify-center rounded border border-borderNeutral bg-white px-3 text-sm text-[#4f4f4f]">
        ...
        <div className="flex items-center justify-end gap-4 border-t border-borderNeutral px-6 py-4">
          ...
          <div className="inline-flex h-10 min-w-[150px] items-center justify-center rounded border border-borderNeutral bg-white px-4 text-base font-semibold text-[#4f4f4f]">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          <button
            type="button"
            onClick={onNavigateToCotacaoGanha}
          ...
          >
            {t("common.cotacao-ganha")}
          </button>
          <button
            type="button"
            onClick={onNavigateToCotacaoReprovada}
          ...
          <button
            type="button"
            onClick={onNavigateToNegociacao}
            className="rounded-full border border-yellow-500 bg-white px-10 py-3 text-sm font-semibold uppercase tracking-[0.1em] text-yellow-500 transition hover:bg-yellow-500/10"
          >
            {t("common.negociacao")}
          </button>
        </div>
