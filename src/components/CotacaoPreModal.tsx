...
const initialProducts: Product[] = [
  ...
  {
    id: "pm1272",
    name: "PMI1272_MARCAPASSO CAMARA ÚNICA ASSURIT...",
    quantity: 1,
    priceList: 4501.76,
    priceQuote: 4501.76
  }
...
export function CotacaoPreModal({
  open,
  onClose,
  onSubmit
}: CotacaoPreModalProps) {
  const { t } = useLanguage();
    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-borderNeutral px-6 py-4">
          <h2 className="text-2xl font-medium text-primary">{t("modal.cotacao-pre")}</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="flex items-center justify-between border-t border-borderNeutral px-6 py-4 gap-4">
            <button
              type="button"
              onClick={onClose}
              className="rounded-lg bg-gray-200 px-8 py-3 text-sm font-semibold uppercase tracking-[0.1em] text-[#4f4f4f] transition hover:bg-gray-300"
            >
              {t("common.voltar")}
            </button>
